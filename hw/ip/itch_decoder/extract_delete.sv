// extract_delete.sv — ITCH 'D' (Order Delete) field extractor
// Absorbs 3 beats (24 bytes, covering D = 19 B), assembles book_event_t.

`ifndef EXTRACT_DELETE_SV
`define EXTRACT_DELETE_SV

`include "book_event_pkg.sv"

module extract_delete #(
    ...
    // Input AXI-S: ITCH 'D' message body, post type-dispatch
    ...
    // 192-bit buffer holds up to 3 beats (24 bytes, covers D = 19 B)
    ...
    //
    // D message layout (19 bytes, 0-indexed):
    //   byte  0     : 'D' (type, already consumed by dispatcher)
...
endmodule : extract_delete

`endif // EXTRACT_DELETE_SV
