"""TB for hw/ip/itch_decoder/extract_cancel.sv."""
from __future__ import annotations
import cocotb
from cocotb.clock import Clock
from cocotb.triggers import RisingEdge
import _axis
from synthetic_gen import _encode_itch, BookEvent, EV_CANCEL
...
@cocotb.test()
async def test_cancel_basic(dut):
    cocotb.start_soon(Clock(dut.clk, 4, units="ns").start())
    await _reset(dut)
    ev = BookEvent(EV_CANCEL, 0, 99, 0, 30, 0xDEADBEEF12345678, 0xCAFE)
    msg = _encode_itch(ev)
    assert msg[:1] == b"X" and len(msg) == 23
...
@cocotb.test()
async def test_cancel_max_shares(dut):
    cocotb.start_soon(Clock(dut.clk, 4, units="ns").start())
    await _reset(dut)
    ev = BookEvent(EV_CANCEL, 0, 1, 0, 0xFFFFFFFF, 0xABCD, 1)
    msg = _encode_itch(ev)
...
@cocotb.test()
async def test_cancel_max_symbol(dut):
    cocotb.start_soon(Clock(dut.clk, 4, units="ns").start())
    await _reset(dut)
    ev = BookEvent(EV_CANCEL, 0, 0xFFFF, 0, 1, 0xCAFEBABE, 0xBEEF)
    msg = _encode_itch(ev)
    event_int = await _drive_and_capture(dut, msg, tuser=0xBEEF)
    f = _unpack(event_int)
